...
ma.fillParticleList(decayString='e+:uncorrected',
                 cut=impactcut + ' and electronID > 0.5', path=main)
...
ma.reconstructDecay('B0 -> J/psi:ee K_S0:merged', cut='Mbc > 5.2 and abs(deltaE) < 0.3', path=main)
...
b_vars += vu.create_aliases_for_selected(cmskinematics, '^B0 -> [^J/psi -> ^e+ ^e-] [^K_S0 -> ^pi+ ^pi-]')
...
ma.variablesToNtuple('B0', variables=b_vars, filename='Bd2JpsiKS_ee.root', treename='tree', path=main)
